...
export async function v1Routes(app: FastifyInstance, providers: ProviderRegistry): Promise<void> {
  ...
  app.post('/investments/orders',{preHandler:[authenticated,requireIdempotencyKey],schema:{body:objectSchema(['simulationId','termsAccepted'],{simulationId:idSchema,termsAccepted:boolean})}},async request=>envelope(request,await providers.investment!.createOrder(request.auth!,request.body,request.headers['idempotency-key'] as string,request.id)));
  app.get('/investments/orders/:id',{preHandler:authenticated,schema:{params:idParams}},async request=>envelope(request,await providers.investment!.getOrder(request.auth!,(request.params as any).id)));
  app.get('/investments/positions',{preHandler:authenticated},async request=>envelope(request,await providers.investment!.listPositions(request.auth!)));
  app.get('/investments/orders/:id/receipt',{preHandler:authenticated,schema:{params:idParams}},async request=>envelope(request,await providers.investment!.getReceipt(request.auth!,(request.params as any).id,request.id)));

  app.get('/billing/payers',{preHandler:authenticated},async request=>envelope(request,await providers.billing!.listPayers(request.auth!)));
  app.post('/billing/payers',{preHandler:[authenticated,requireIdempotencyKey],schema:{body:objectSchema(['name'],{name:string})}},async request=>envelope(request,await providers.billing!.createPayer(request.auth!,request.body,request.headers['idempotency-key'] as string)));
  app.put('/billing/payers/:id',{preHandler:authenticated,schema:{params:idParams,body:objectSchema(['name'],{name:string})}},async request=>envelope(request,await providers.billing!.updatePayer(request.auth!,(request.params as any).id,request.body)));
  app.delete('/billing/payers/:id',{preHandler:authenticated,schema:{params:idParams}},async request=>envelope(request,await providers.billing!.deletePayer(request.auth!,(request.params as any).id)));
  app.get('/billing/limits',{preHandler:authenticated},async request=>envelope(request,await providers.billing!.getLimits(request.auth!)));
  app.get('/billing/bills',{preHandler:authenticated},async request=>envelope(request,await providers.billing!.listBills(request.auth!)));
  app.post('/billing/bills',{preHandler:[authenticated,requireIdempotencyKey],schema:{body:objectSchema(['payerId','dueDate','amountMinor'],{payerId:idSchema,dueDate:string,amountMinor:{type:'integer'},description:string,finePercentSandbox:{type:'number'},interestPercentSandbox:{type:'number'}})}},async request=>envelope(request,await providers.billing!.createBill(request.auth!,request.body,request.headers['idempotency-key'] as string,request.id)));
  app.get('/billing/bills/:id',{preHandler:authenticated,schema:{params:idParams}},async request=>envelope(request,await providers.billing!.getBill(request.auth!,(request.params as any).id)));
  app.post('/billing/bills/:id/share',{preHandler:authenticated,schema:{params:idParams,body:emptyObjectSchema}},async request=>envelope(request,await providers.billing!.shareBill(request.auth!,(request.params as any).id)));
  app.get('/billing/bills/:id/receipt',{preHandler:authenticated,schema:{params:idParams}},async request=>envelope(request,await providers.billing!.getReceipt(request.auth!,(request.params as any).id,request.id)));

  app.get('/consigned/products',{preHandler:authenticated},async request=>envelope(request,await providers.consigned!.listProducts(request.auth!)));
  app.get('/consigned/products/:id/documents',{preHandler:authenticated,schema:{params:idParams}},async request=>envelope(request,await providers.consigned!.getDocuments(request.auth!,(request.params as any).id)));
  app.get('/consigned/products/:id/terms',{preHandler:authenticated,schema:{params:idParams}},async request=>envelope(request,await providers.consigned!.getTerms(request.auth!,(request.params as any).id)));
  app.post('/consigned/applications',{preHandler:[authenticated,requireIdempotencyKey],schema:{body:objectSchema(['productId','amountMinor','installments','termsAccepted','documentsAcknowledged'],{productId:idSchema,amountMinor:{type:'integer'},installments:{type:'integer'},termsAccepted:boolean,documentsAcknowledged:boolean})}},async request=>envelope(request,await providers.consigned!.apply(request.auth!,request.body,request.headers['idempotency-key'] as string,request.id)));
  app.get('/consigned/applications',{preHandler:authenticated},async request=>envelope(request,await providers.consigned!.listApplications(request.auth!)));
  app.get('/consigned/applications/:id',{preHandler:authenticated,schema:{params:idParams}},async request=>envelope(request,await providers.consigned!.getApplication(request.auth!,(request.params as any).id)));
  app.get('/consigned/applications/:id/receipt',{preHandler:authenticated,schema:{params:idParams}},async request=>envelope(request,await providers.consigned!.getReceipt(request.auth!,(request.params as any).id,request.id)));
