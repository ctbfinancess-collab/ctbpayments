export interface ChangeEmailTemplateInput {
  confirmUrl: string;
...
// Mesma identidade visual e mesma estrutura de tabelas de
// verifyEmailTemplate.ts/resetPasswordTemplate.ts — só muda o texto/CTA.
// Enviado sempre para o NOVO endereço (nunca para o antigo) — é a prova
// de posse que autoriza a troca.
export function renderChangeEmailTemplate(input: ChangeEmailTemplateInput): RenderedEmail {
  const { confirmUrl, expiresInMinutes } = input;
  const subject = 'Confirme seu novo e-mail — CTBX Payments';
...
<html lang="pt-BR">
          ...
          <table role="presentation" align="center" width="480" class="ctbx-email-container" style="width:480px; max-width:480px;" cellpadding="0" cellspacing="0">
            ...
            <tr>
              <td class="ctbx-email-card" style="background-color:#ffffff; border-radius:14px; border-top:3px solid #FF7A3D; padding:28px 28px; box-shadow:0 16px 40px rgba(0,0,0,0.3);">
                ...
                <h1 style="margin:16px 0 4px; font-size:18px; color:#161B33;">Confirme seu novo e-mail</h1>
                ...
                <p style="margin:0 0 18px; font-size:13px; line-height:1.55; color:#4B5573;">
                  Recebemos um pedido para usar este endereço como o novo e-mail da sua conta no <strong style="color:#161B33;">CTBX Payments</strong>.
                  Clique no botão abaixo para confirmar a troca.
                ...
                <table role="presentation" cellpadding="0" cellspacing="0" style="margin:0 auto 20px;">
                  <tr>
                    <td align="center" style="border-radius:8px; background:linear-gradient(90deg,#5B3FE0,#FF7A3D);">
                      <a href="${confirmUrl}" style="display:inline-block; padding:12px 26px; font-size:13px; font-weight:700; color:#ffffff; text-decoration:none;">CONFIRMAR NOVO E-MAIL</a>
                    </td>
                  </tr>
                </table>
                <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color:#F5F6FB; border-radius:10px; padding:14px 16px;">
                  ...
                  <tr><td style="font-size:12px; color:#4B5573; padding:2px 0;">&#10003; Se você não pediu essa troca, ignore este e-mail — nada será alterado.</td></tr>
                </table>
                <p style="margin:18px 0 0; font-size:11px; color:#8B93AB;">Se o botão não funcionar, copie e cole este link no navegador:<br /><span style="word-break:break-all; color:#5B3FE0;">${confirmUrl}</span></p>
  ...
  const text = [
    'Confirme seu novo e-mail — CTBX Payments',
    '',
    'Olá!',
    '',
    'Recebemos um pedido para usar este endereço como o novo e-mail da sua conta no CTBX Payments. Acesse o link abaixo para confirmar a troca.',
    ...
    '- Se você não pediu essa troca, ignore este e-mail — nada será alterado.',
    '',
    'Atenciosamente,',
