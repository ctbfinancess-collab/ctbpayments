...
// Mesma identidade visual e mesma estrutura de tabelas do
// verifyEmailTemplate.ts (largura fixa em pixel, media query pra
// encolher no mobile) — só muda o texto/ícone/CTA. Sem imagem externa,
// sem nenhum dado sensível: só o link de reset (o token só existe como
// query param, nunca em texto explicado/rotulado).
export function renderResetPasswordTemplate(input: ResetPasswordTemplateInput): RenderedEmail {
  const { resetUrl, expiresInMinutes } = input;
  const subject = 'Redefinição de senha — CTBX Payments';
...
<html lang="pt-BR">
            ...
            <tr>
              <td class="ctbx-email-card" style="background-color:#ffffff; border-radius:14px; border-top:3px solid #FF7A3D; padding:28px 28px; box-shadow:0 16px 40px rgba(0,0,0,0.3);">
                <table role="presentation" cellpadding="0" cellspacing="0">
                  <tr>
                    <td style="width:40px; height:40px; border-radius:50%; background-color:#F1EEFE; text-align:center; vertical-align:middle; font-size:18px; line-height:40px;">&#128274;</td>
                  </tr>
                </table>
                <h1 style="margin:16px 0 4px; font-size:18px; color:#161B33;">Redefinir sua senha</h1>
                ...
                <p style="margin:0 0 18px; font-size:13px; line-height:1.55; color:#4B5573;">
                  Recebemos um pedido para redefinir a senha da sua conta no <strong style="color:#161B33;">CTBX Payments</strong>.
                ...
                <table role="presentation" cellpadding="0" cellspacing="0" style="margin:0 auto 20px;">
                  <tr>
                    <td align="center" style="border-radius:8px; background:linear-gradient(90deg,#5B3FE0,#FF7A3D);">
                      <a href="${resetUrl}" style="display:inline-block; padding:12px 26px; font-size:13px; font-weight:700; color:#ffffff; text-decoration:none;">REDEFINIR MINHA SENHA</a>
                    </td>
                  </tr>
                </table>
                <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color:#F5F6FB; border-radius:10px; padding:14px 16px;">
                  ...
                  <tr><td style="font-size:12px; color:#4B5573; padding:2px 0;">&#10003; Se você não pediu essa alteração, ignore este e-mail — sua senha continua a mesma.</td></tr>
                </table>
                <p style="margin:18px 0 0; font-size:11px; color:#8B93AB;">Se o botão não funcionar, copie e cole este link no navegador:<br /><span style="word-break:break-all; color:#5B3FE0;">${resetUrl}</span></p>
  ...
  const text = [
    'Redefinição de senha — CTBX Payments',
    '',
    'Olá!',
    '',
    'Recebemos um pedido para redefinir a senha da sua conta no CTBX Payments. Acesse o link abaixo para escolher uma nova senha.',
