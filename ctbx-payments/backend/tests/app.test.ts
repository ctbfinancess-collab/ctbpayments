...
test('investment SANDBOX completes products, simulation, idempotent order, position and receipt',async t=>{const app=await buildApp({config:testConfig,logger:false});t.after(()=>app.close());const s=await login(app),h=authHeaders(s);assert.equal((await app.inject({method:'GET',url:'/v1/investments/products'})).statusCode,401);const ps=(await app.inject({method:'GET',url:'/v1/investments/products',headers:h})).json().data,p=ps[0];const sim=await app.inject({method:'POST',url:'/v1/investments/simulations',headers:h,payload:{productId:p.id,amountMinor:20000}});assert.equal(sim.statusCode,200);assert.equal(Number.isInteger(sim.json().data.projectedNetMinor),true);const headers={...h,'idempotency-key':'idem-investment-0001'},payload={simulationId:sim.json().data.simulationId,termsAccepted:true};const a=await app.inject({method:'POST',url:'/v1/investments/orders',headers,payload}),b=await app.inject({method:'POST',url:'/v1/investments/orders',headers,payload});assert.equal(a.json().data.orderId,b.json().data.orderId);const conflict=await app.inject({method:'POST',url:'/v1/investments/orders',headers,payload:{...payload,termsAccepted:false}});assert.equal(conflict.statusCode,409);assert.equal((await app.inject({method:'GET',url:'/v1/investments/positions',headers:h})).json().data.length,1);assert.equal((await app.inject({method:'GET',url:`/v1/investments/orders/${a.json().data.orderId}/receipt`,headers:h})).json().data.simulated,true);});
