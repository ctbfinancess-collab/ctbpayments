...
export function TransferToggle({ label, onPress, selected }) {
  return <TouchableOpacity onPress={onPress} style={styles.toggle}><View style={[styles.checkbox, selected && styles.checked]}><Text style={styles.check}>{selected ? '✓' : ''}</Text></View><Text style={styles.toggleText}>{label}</Text></TouchableOpacity>;
}

const styles = StyleSheet.create({
  group: { marginBottom: spacing.lg }, label: { ...typography.label, color: colors.textSecondary, marginBottom: spacing.sm },
  input: { backgroundColor: colors.surfaceElevated, borderColor: colors.borderSubtle, borderRadius: radii.md, borderWidth: 1, color: colors.textPrimary, fontSize: 14, minHeight: 52, paddingHorizontal: spacing.lg, paddingVertical: spacing.md },
  focused: { borderColor: colors.purple500, borderWidth: 1.5 }, multiline: { minHeight: 88, textAlignVertical: 'top' },
  info: { borderBottomColor: colors.borderSubtle, borderBottomWidth: 1, paddingVertical: spacing.md }, infoLast: { borderBottomWidth: 0 },
