...
export default function CardActivationScreen({ navigation }) {
  ...
  return <CardScreenLayout navigation={navigation} title="Ativar cartão"><View style={styles.wrap}><Text style={styles.step}>ETAPA {Math.min(step + 1, 4)} DE 4</Text><Card><Text style={styles.title}>{['Confirme seus dados', 'Código por SMS', 'Crie a senha do cartão', 'Cartão ativado'][step]}</Text><Text style={styles.copy}>{['Enviaremos um código para o celular cadastrado.', 'Digite o código SANDBOX 123456.', 'A senha deve ter quatro números.', 'COMPROVANTE SANDBOX · OPERAÇÃO SIMULADA'][step]}</Text>{step < 2 ? <TextInput value={value} onChangeText={setValue} keyboardType="number-pad" placeholder={step ? 'Código SMS' : '4 últimos dígitos'} placeholderTextColor={colors.textMuted} style={styles.input} /> : null}{step === 2 ? <><TextInput value={pass} onChangeText={setPass} secureTextEntry keyboardType="number-pad" placeholder="Nova senha" placeholderTextColor={colors.textMuted} style={styles.input} /><TextInput value={confirm} onChangeText={setConfirm} secureTextEntry keyboardType="number-pad" placeholder="Confirmar senha" placeholderTextColor={colors.textMuted} style={styles.input} /></> : null}</Card>{step < 3 ? <PrimaryButton disabled={loading} loading={loading} title="Continuar" onPress={next} /> : <ConfirmationButton title="Voltar aos cartões" onPress={() => navigation.navigate('Cards')} />}</View></CardScreenLayout>;
}
const styles = StyleSheet.create({ wrap: { gap: spacing.lg, padding: spacing.lg }, step: { ...typography.caption, color: colors.purple400 }, title: { ...typography.heading2, color: colors.textPrimary }, copy: { ...typography.body, color: colors.textSecondary, marginVertical: spacing.md }, input: { backgroundColor: colors.surfaceElevated, borderColor: colors.border, borderRadius: 14, borderWidth: 1, color: colors.textPrimary, marginTop: spacing.sm, padding: spacing.md } });
