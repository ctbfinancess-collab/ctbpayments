...
import { BalanceCard, Card, EmptyState, ErrorState, LoadingState, OutlineButton, PrimaryButton, SectionTitle } from '../../components/ui';
...
export default function StatementScreen({ navigation, route }) {
  ...
  if (loading) return <StatementLayout navigation={navigation} title="Extrato"><LoadingState /></StatementLayout>;
  if (error) return <StatementLayout navigation={navigation} title="Extrato"><ErrorState message="Não foi possível carregar o extrato." onRetry={retry} /></StatementLayout>;
  return <StatementLayout navigation={navigation} title="Extrato">
    <BalanceCard label="Saldo da conta digital" onToggleVisibility={() => setVisibleBalance((v) => !v)} value={statementData.balance} variant="blue" visible={visibleBalance} />
