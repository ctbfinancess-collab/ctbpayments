...
export default function TransferAuthorizationScreen({ navigation, route }) {
  const transfer = route.params?.transfer; const [password, setPassword] = useState(''); const [otp, setOtp] = useState(''); const [requested, setRequested] = useState(false); if (!transfer) return <MissingDataState navigation={navigation} title="Autorizar transferência" />;
  const request = () => { setRequested(true); Alert.alert('Token de demonstração', 'Digite qualquer código de 6 dígitos.'); };
  const authorize = () => { if (password.length < 4) return Alert.alert('Digite sua senha'); if (!requested) return request(); if (!/^\d{6}$/.test(otp)) return Alert.alert('Token inválido'); navigation.replace('TransferReceipt', { transfer }); };
  return <TransferLayout navigation={navigation} title="Autorizar transferência"><Text style={styles.value}>R$ {transfer.amount}</Text><Text style={styles.recipient}>Para {transfer.beneficiary.name}</Text><TransferField keyboardType="numeric" label="Senha de transferência" maxLength={6} onChangeText={setPassword} secureTextEntry value={password} />{requested ? <TransferField keyboardType="numeric" label="Token" maxLength={6} onChangeText={setOtp} value={otp} /> : <OutlineButton onPress={request}>Enviar token</OutlineButton>}<ConfirmationButton onPress={authorize} style={styles.confirm}>Transferir</ConfirmationButton><Text style={styles.mock}>Senha, token, biometria e envio bancário simulados.</Text></TransferLayout>;
}
