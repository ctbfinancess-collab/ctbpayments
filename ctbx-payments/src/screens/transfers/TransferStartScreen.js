...
export default function TransferStartScreen({ navigation }) {
  return <TransferLayout navigation={navigation} title="Transferências">
    ...
    <View style={styles.row}>
      <ServiceCard icon={<Text style={styles.icon}>↶</Text>} label="Entre contas CTBX" onPress={() => navigation.navigate('TransferBeneficiary', { mode: 'internal' })} style={styles.option} />
      <ServiceCard icon={<Text style={styles.icon}>↔</Text>} label="Para outros bancos" onPress={() => navigation.navigate('TransferBeneficiary', { mode: 'external' })} style={styles.option} />
    </View>
    <Text style={styles.note}>Fluxo original: NavScreen68 → NavScreen69 / NavScreen6</Text>
  </TransferLayout>;
}
const styles = StyleSheet.create({
  hero: { alignItems: 'center', flexDirection: 'row', padding: spacing.xl }, heroIcon: { color: colors.orange400, fontSize: 32 }, copy: { flex: 1, marginLeft: spacing.lg }, title: { ...typography.heading2, color: colors.textPrimary }, subtitle: { ...typography.body, color: colors.textSecondary, marginTop: 4 }, section: { marginBottom: spacing.md, marginTop: spacing.xxl }, row: { flexDirection: 'row', gap: spacing.md }, option: { flex: 1 }, icon: { color: colors.purple300, fontSize: 24 }, note: { ...typography.caption, color: colors.textMuted, marginTop: spacing.xxl, textAlign: 'center' },
});
